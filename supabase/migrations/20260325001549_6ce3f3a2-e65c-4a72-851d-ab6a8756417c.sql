...
BEGIN
  ...
  SELECT
    ...
    END,
    -- Return customer_phone ONLY after acceptance
    CASE
      WHEN b.accepted_at IS NOT NULL AND b.assigned_provider_id = auth.uid()
    ...
    b.calculated_total,
    NULL::text  -- OTP is NEVER exposed to provider; verified server-side only
