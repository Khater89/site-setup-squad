...
CREATE FUNCTION public.available_bookings_for_providers()
 RETURNS TABLE(id uuid, service_id text, city text, scheduled_at timestamp with time zone, booking_number text, area_public text, notes text, created_at timestamp with time zone, payment_method text)
 LANGUAGE sql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
  SELECT b.id, b.service_id, b.city, b.scheduled_at, b.booking_number, b.area_public, b.notes, b.created_at, b.payment_method
  FROM public.bookings b
  WHERE b.status = 'NEW'
    AND is_provider()
